...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public class ConvexHullShape : ConvexShape
    {
        ...
        public ConvexHullShape(IList<Vector3> vertices)
        {
            ...
            var surfaceVertices = BEPUphysics.ResourceManagement.Resources.GetVectorList();
            ComputeCenter(vertices, surfaceVertices);
            this.vertices = new RawList<Vector3>(surfaceVertices);
            BEPUphysics.ResourceManagement.Resources.GiveBack(surfaceVertices);
        ...
        public override void GetBoundingBox(ref RigidTransform shapeTransform, out BoundingBox boundingBox)
        {
            ...
            Vector3Ex.Dot(ref vertices.Elements[0], ref right, out maxX);
            minX = maxX;
            Vector3Ex.Dot(ref vertices.Elements[0], ref up, out maxY);
            minY = maxY;
            Vector3Ex.Dot(ref vertices.Elements[0], ref backward, out maxZ);
            minZ = maxZ;
            int minXIndex = 0;
            int maxXIndex = 0;
            int minYIndex = 0;
            ...
            {
                float dot;
                Vector3Ex.Dot(ref vertices.Elements[i], ref right, out dot);
                if (dot < minX)
                {
                    minX = dot;
                    minXIndex = i;
                ...
                Vector3Ex.Dot(ref vertices.Elements[i], ref up, out dot);
                if (dot < minY)
                {
                    minY = dot;
                    minYIndex = i;
                ...
                Vector3Ex.Dot(ref vertices.Elements[i], ref backward, out dot);
                if (dot < minZ)
                {
                    minZ = dot;
                    minZIndex = i;
            ...
            boundingBox.Maximum.X = shapeTransform.Position.X + collisionMargin + maxXpoint.X;
            boundingBox.Maximum.Y = shapeTransform.Position.Y + collisionMargin + maxYpoint.Y;
            boundingBox.Maximum.Z = shapeTransform.Position.Z + collisionMargin + maxZpoint.Z;

            boundingBox.Minimum.X = shapeTransform.Position.X - collisionMargin + minXpoint.X;
            boundingBox.Minimum.Y = shapeTransform.Position.Y - collisionMargin + minYpoint.Y;
            boundingBox.Minimum.Z = shapeTransform.Position.Z - collisionMargin + minZpoint.Z;
        }
        ...
        {
            float max;
            Vector3Ex.Dot(ref vertices.Elements[0], ref direction, out max);
            int maxIndex = 0;
            for (int i = 1; i < vertices.count; i++)
            {
                float dot;
                Vector3Ex.Dot(ref vertices.Elements[i], ref direction, out dot);
                if (dot > max)
                {
                    max = dot;
                    maxIndex = i;
        ...
        {
            var localSurfaceVertices = BEPUphysics.ResourceManagement.Resources.GetVectorList();
            var surfaceTriangles = BEPUphysics.ResourceManagement.Resources.GetIntList();
            Vector3 toReturn = ComputeCenter(vertices, out volume, surfaceTriangles, localSurfaceVertices);
            BEPUphysics.ResourceManagement.Resources.GiveBack(localSurfaceVertices);
            BEPUphysics.ResourceManagement.Resources.GiveBack(surfaceTriangles);
            return toReturn;
        }
        ...
        {
            float volume;
            var indices = BEPUphysics.ResourceManagement.Resources.GetIntList();
            Vector3 toReturn = ComputeCenter(vertices, out volume, indices, outputLocalSurfaceVertices);
            BEPUphysics.ResourceManagement.Resources.GiveBack(indices);
            return toReturn;
        }
        ...
        public static Vector3 ComputeCenter(IList<Vector3> vertices, out float volume, IList<int> outputSurfaceTriangles, IList<Vector3> outputLocalSurfaceVertices)
        {
            ...
            volume = 0;
            var volumes = BEPUphysics.ResourceManagement.Resources.GetFloatList();
            var centroids = BEPUphysics.ResourceManagement.Resources.GetVectorList();
            ...
            BEPUphysics.ResourceManagement.Resources.GiveBack(centroids);
            BEPUphysics.ResourceManagement.Resources.GiveBack(volumes);
            return center;
        }
        ...
        public override float ComputeMinimumRadius()
        {
            ...
            Vector3Ex.Dot(ref a, ref nABC, out dABC);
            Vector3Ex.Dot(ref c, ref nCBD, out dCBD);
            Vector3Ex.Dot(ref a, ref nACD, out dACD);
            Vector3Ex.Dot(ref a, ref nADB, out dADB);
            dABC /= nABC.Length();
            dCBD /= nCBD.Length();
            dACD /= nACD.Length();
            dADB /= nADB.Length();
