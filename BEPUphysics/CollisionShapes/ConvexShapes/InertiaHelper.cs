...
namespace BEPUphysics.CollisionShapes.ConvexShapes
{
    ...
    public class InertiaHelper
    {
        ...
        {
            var pointContributions = BEPUphysics.ResourceManagement.Resources.GetVectorList();
            GetPoints(shape, out volume, pointContributions);
            Vector3 center = AveragePoints(pointContributions);
            BEPUphysics.ResourceManagement.Resources.GiveBack(pointContributions);
            return center;
        }
        ...
        public static void GetPoints(ConvexShape shape, out float volume, RawList<Vector3> outputPointContributions)
        {
            ...
            //XY -> use Z
            Ray ray;
            float width = boundingBox.Maximum.X - boundingBox.Minimum.X;
            float height = boundingBox.Maximum.Y - boundingBox.Minimum.Y;
            float length = boundingBox.Maximum.Z - boundingBox.Minimum.Z;
            ...
            if (yzArea > xzArea && yzArea > xyArea)
            {
                ...
                ray.Position = new Vector3(boundingBox.Minimum.X, boundingBox.Minimum.Y + .5f * incrementMultiplier * height, boundingBox.Minimum.Z + .5f * incrementMultiplier * length);
            ...
            else if (xzArea > xyArea) //yz is not the max, given by the previous if.  Is xz or xy the max?
            {
                ...
                ray.Position = new Vector3(boundingBox.Minimum.X + .5f * incrementMultiplier * width, boundingBox.Minimum.Y, boundingBox.Minimum.Z + .5f * incrementMultiplier * length);
            ...
            else
            {
                ...
                ray.Position = new Vector3(boundingBox.Minimum.X + .5f * incrementMultiplier * width, boundingBox.Minimum.Y + .5f * incrementMultiplier * height, boundingBox.Minimum.Z);
                increment1 = new Vector3(incrementMultiplier * width, 0, 0);
