...
namespace BEPUphysics.CollisionShapes
{
    ...
    public class MobileMeshShape : EntityShape
    {
        ...
        {
            var overlapList = BEPUphysics.ResourceManagement.Resources.GetIntList();
            hit = new RayHit();
            ...
            if (triangleMesh.Tree.GetOverlaps(ray, overlapList))
            {
                ...
                BEPUphysics.ResourceManagement.Resources.GiveBack(overlapList);
        ...
        TriangleSidedness ComputeSolidSidednessHelper(Ray ray)
        {
            ...
            var hitList = BEPUphysics.ResourceManagement.Resources.GetIntList();
            if (triangleMesh.Tree.GetOverlaps(ray, hitList))
            {
                Vector3 vA, vB, vC;
                var hits = BEPUphysics.ResourceManagement.Resources.GetRayHitList();
                ...
                BEPUphysics.ResourceManagement.Resources.GiveBack(hits);

            }
            else
                toReturn = TriangleSidedness.DoubleSided; //This is a problem...
            BEPUphysics.ResourceManagement.Resources.GiveBack(hitList);
            return toReturn;
        }
        ...
        void ComputeShapeInformation(TransformableMeshData data, out ShapeDistributionInformation shapeInformation)
        {
            ...
            catch
            {
                ...
                for (int i = 0; i < data.vertices.Length; i++)
                {
                    ...
                    if (v.X > box.Maximum.X)
                        box.Maximum.X = v.X;
                    if (v.X < box.Minimum.X)
                        box.Minimum.X = v.X;
                    if (v.Y > box.Maximum.Y)
                        box.Maximum.Y = v.Y;
                    if (v.Y < box.Minimum.Y)
                        box.Minimum.Y = v.Y;
                    if (v.Z > box.Maximum.Z)
                        box.Maximum.Z = v.Z;
                    if (v.Z < box.Minimum.Z)
                        box.Minimum.Z = v.Z;
                }
                //Add the corners.  This will overestimate the size of the surface a bit.
                surfaceVertices.Add(box.Minimum);
                surfaceVertices.Add(box.Maximum);
                surfaceVertices.Add(new Vector3(box.Minimum.X, box.Minimum.Y, box.Maximum.Z));
                surfaceVertices.Add(new Vector3(box.Minimum.X, box.Maximum.Y, box.Minimum.Z));
                surfaceVertices.Add(new Vector3(box.Maximum.X, box.Minimum.Y, box.Minimum.Z));
                surfaceVertices.Add(new Vector3(box.Minimum.X, box.Maximum.Y, box.Maximum.Z));
                surfaceVertices.Add(new Vector3(box.Maximum.X, box.Maximum.Y, box.Minimum.Z));
                surfaceVertices.Add(new Vector3(box.Maximum.X, box.Minimum.Y, box.Maximum.Z));
            }
        ...
        //    {
        //        float dot;
        //        Vector3Ex.Dot(ref v, ref Direction, out dot);
        //        float difference;
        //        if (dot < Minimum)
        //        {
        ...
        //        {
        //            float dot;
        //            Vector3Ex.Dot(ref v, ref extents.Elements[j].Direction, out dot);
        ...
        private void GetBoundingBox(ref Matrix3X3 o, out BoundingBox boundingBox)
        {
            ...
            for (int i = 0; i < surfaceVertices.count; i++)
            {
                ...
                Vector3Ex.Dot(ref rightDirection, ref surfaceVertices.Elements[i], out dotX);
                Vector3Ex.Dot(ref upDirection, ref surfaceVertices.Elements[i], out dotY);
                Vector3Ex.Dot(ref backDirection, ref surfaceVertices.Elements[i], out dotZ);
                if (dotX < minX)
                {
                    minX = dotX;
                    left = i;
            ...
            boundingBox.Maximum.X = vMaxX.X;
            boundingBox.Maximum.Y = vMaxY.Y;
            boundingBox.Maximum.Z = vMaxZ.Z;

            boundingBox.Minimum.X = vMinX.X;
            boundingBox.Minimum.Y = vMinY.Y;
            boundingBox.Minimum.Z = vMinZ.Z;
        }
        ...
        public void GetBoundingBox(ref RigidTransform shapeTransform, out BoundingBox boundingBox)
        {
            ...
            //boundingBox.Maximum.X = shapeTransform.Position.X + right.X;
            //boundingBox.Maximum.Y = shapeTransform.Position.Y + up.Y;
            //boundingBox.Maximum.Z = shapeTransform.Position.Z + backward.Z;

            //boundingBox.Minimum.X = shapeTransform.Position.X + left.X;
            //boundingBox.Minimum.Y = shapeTransform.Position.Y + down.Y;
            //boundingBox.Minimum.Z = shapeTransform.Position.Z + forward.Z;
            ...
            boundingBox.Maximum.X += shapeTransform.Position.X;
            boundingBox.Maximum.Y += shapeTransform.Position.Y;
            boundingBox.Maximum.Z += shapeTransform.Position.Z;

            boundingBox.Minimum.X += shapeTransform.Position.X;
            boundingBox.Minimum.Y += shapeTransform.Position.Y;
            boundingBox.Minimum.Z += shapeTransform.Position.Z;
        ...
        public void GetLocalBoundingBox(ref RigidTransform shapeTransform, ref AffineTransform spaceTransform, out BoundingBox boundingBox)
        {
            ...
            boundingBox.Maximum.X += transform.Translation.X;
            boundingBox.Maximum.Y += transform.Translation.Y;
            boundingBox.Maximum.Z += transform.Translation.Z;

            boundingBox.Minimum.X += transform.Translation.X;
            boundingBox.Minimum.Y += transform.Translation.Y;
            boundingBox.Minimum.Z += transform.Translation.Z;
