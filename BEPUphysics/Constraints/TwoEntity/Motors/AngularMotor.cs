...
namespace BEPUphysics.Constraints.TwoEntity.Motors
{
    ...
    public class AngularMotor : Motor, I3DImpulseConstraintWithError, I3DJacobianConstraint
    {
        ...
        public AngularMotor(Entity connectionA, Entity connectionB)
        {
            ...
            QuaternionEx.Multiply(ref this.connectionA.orientation, ref orientationBConjugate, out settings.servo.goal);
        }
        ...
        public override void Update(float dt)
        {
            ...
            if (settings.mode == MotorMode.Servomechanism) //Only need to do the bulk of this work if it's a servo.
            {
                ...
                //Quaternion basis;
                //Quaternion.RotationMatrix(ref worldTransform, out basis);

                //Quaternion currentRelativeOrientation;
                //QuaternionEx.Multiply(ref basis, ref conjugateQuaternionB, out currentRelativeOrientation);
                ...
                //QuaternionEx.Multiply(ref mySettings.myServo.myGoal, ref basisConjugate, out goal);
                //QuaternionEx.Multiply(ref basis, ref goal, out goal);
                ...
                //QuaternionEx.Multiply(ref currentRelativeOrientation, ref goal, out errorOrientation);


                Matrix worldTransform = Matrix3X3.ToMatrix4X4(basis.WorldTransform);
                Quaternion basisOrientation;
                Quaternion.RotationMatrix(ref worldTransform, out basisOrientation);

                Quaternion quaternionB;
                Quaternion.Conjugate(ref connectionB.orientation, out quaternionB);
                Quaternion errorOrientation;
                QuaternionEx.Multiply(ref basisOrientation, ref quaternionB, out errorOrientation);

                //Construct the goal in world space using the basis.
                Quaternion goal;
                Quaternion basisConjugate;
                Quaternion.Conjugate(ref basisOrientation, out basisConjugate);
                QuaternionEx.Multiply(ref settings.servo.goal, ref basisConjugate, out goal);
                QuaternionEx.Multiply(ref basisOrientation, ref goal, out goal);

                QuaternionEx.Multiply(ref goal, ref errorOrientation, out errorOrientation);
