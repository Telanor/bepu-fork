    ...
    class GridCell2D
    {
        ...
        internal int GetIndex(float x)
        {
            ...
            while (maxIndex - minIndex > 0)
            {
                ...
                if (entries.Elements[index].item.boundingBox.Minimum.X > x)
                    maxIndex = index;
                else if (entries.Elements[index].item.boundingBox.Minimum.X < x)
                    minIndex = ++index;
                else
        ...
        internal void Add(Grid2DEntry entry)
        {
            ...
            entries.Insert(GetIndex(entry.item.boundingBox.Minimum.X), entry);
        }
        ...
        internal void UpdateOverlaps(Grid2DSortAndSweep owner)
        {
            ...
            for (int i = 1; i < entries.count; i++)
            {
                ...
                for (int j = i - 1; j >= 0; j--)
                {
                    if (entry.item.boundingBox.Minimum.X < entries.Elements[j].item.boundingBox.Minimum.X)
                    {
            ...
            for (int i = 0; i < entries.count; i++)
            {
                ...
                Grid2DEntry b;
                //TODO: Microoptimize
                for (int j = i + 1; j < entries.count && a.item.boundingBox.Maximum.X >= (b = entries.Elements[j]).item.boundingBox.Minimum.X; j++)
                {
                    if (!(a.item.boundingBox.Minimum.Y > b.item.boundingBox.Maximum.Y || a.item.boundingBox.Maximum.Y < b.item.boundingBox.Minimum.Y ||
                          a.item.boundingBox.Minimum.Z > b.item.boundingBox.Maximum.Z || a.item.boundingBox.Maximum.Z < b.item.boundingBox.Minimum.Z))
                    {
