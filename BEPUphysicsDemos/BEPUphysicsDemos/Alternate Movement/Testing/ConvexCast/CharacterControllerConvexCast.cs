    ...
    public class CharacterControllerConvexCast : Updateable, IBeforeNarrowPhaseUpdateable, IEndOfTimeStepUpdateable
    {
        ...
        void ExpandBoundingBox()
        {
            ...
            convexCastDownVolume = new BoundingBox()
                {
                    Minimum = Body.Position + new Vector3(-radius, -Body.Length * .5f - radius - StepHeight * 2 - supportMargin, -radius),
                    Maximum = Body.Position + new Vector3(radius, -Body.Length * .5f, radius)
                };
            convexCastUpVolume = new BoundingBox()
            {
                Minimum = Body.Position + new Vector3(-radius, Body.Length * .5f, -radius),
                Maximum = Body.Position + new Vector3(radius, Body.Length * .5f + radius + StepHeight, radius)
            };
        }
        ...
        void IBeforeNarrowPhaseUpdateable.Update(float dt)
        {
            ...
            {
                if (collidable.BoundingBox.Intersects(ref convexCastDownVolume))
                {
                    RayHit hit;
            ...
            if (IsSupported)
            {
                ...
                if (MovementDirection.LengthSquared() > 0)
                {
                    ...
                    Vector3Ex.Dot(ref velocityDirection, ref relativeVelocity, out currentVelocity);
